    ...
    <div class="row">
      <div class="col-lg-6 col-md-6">
        <div class="form-group">
          <button type="button" class="btn mb-1 btn-info btn-lg btn-block pull-up" onclick="add_person()"> <a> <i class="la la-plus"></i> Tambah Detail Produk Jasa Baru </a></button>
        </div>
      </div>
      ...
      <div class="col-lg-6 col-md-6">
        <div class="form-group">
          <a class="btn mb-1 btn-info btn-lg btn-block pull-up" href="<?php echo site_url('master/produkjasa')?>"><i class="la la-fast-backward"></i> Kembali</a>
        </div>
      </div>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('master/produkjasa/ajax_list_detail/'.$id_header)?>",
            "type": "POST"
        },
...
function add_person()
{
    ...
    $('.modal-title').text('Tambah Produk Jasa Baru'); // Set Title to Bootstrap modal title
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('master/produkjasa/ajax_edit_detail')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.id_detail_layanan);
            $('[name="id_detail_layanan"]').val(data.id_detail_layanan);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master/produkjasa/ajax_add_detail/'.$id_header)?>";
    } else {
        url = "<?php echo site_url('master/produkjasa/ajax_update_detail')?>";
    }
...
function delete_person(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('master/produkjasa/ajax_delete_detail')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<script type="text/javascript">
function detail(ID){
     location.replace("<?php echo base_url('master/produkjasa/detail/')?>"+ID);
                     ...
                     <div class="form-body">
                         <div class="form-group">
                            <div class="row">
                              ...
                              <label class="control-label col-md-12">Pilih Syarat Produk Jasa</label>
                              <div class="col-md-12">
                                 <select class="form-control" name="id_syrt_layanan" required>
                                    <?php foreach ($syarat as $key => $value) { ?>
                                        <option value="<?php echo $value->id_syrt_layanan ?>"> <?php echo $value->nm_syrt_layanan ?> </option>
                                    <?php }?>
                                </select>
                             </div>
                            </div>
