    ...
    <div class="row">
      <div class="col-lg-6 col-md-6">
        <div class="form-group">
          <button type="button" class="btn mb-1 btn-info btn-lg btn-block pull-up" onclick="add_person()"> <a> <i class="la la-plus"></i> Tambah Jenis Rekening Biaya Baru </a></button>
        </div>
      </div>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
      ...
      "order": [], //Initial no order.
        ...
        "ajax": {
            "url": "<?php echo site_url('master/jenisrekeningbiaya/ajax_list')?>",
            "type": "POST"
        },
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('master/jenisrekeningbiaya/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id_jns_rekbiaya"]').val(data.id_jns_rekbiaya);
            $('[name="id_jns_rekbiayaa"]').val(data.id_jns_rekbiaya);
            $('[name="nm_jns_rekbiaya"]').val(data.nm_jns_rekbiaya);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master/jenisrekeningbiaya/ajax_add')?>";
    } else {
        url = "<?php echo site_url('master/jenisrekeningbiaya/ajax_update')?>";
    }
...
function delete_person(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('master/jenisrekeningbiaya/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    if(list_id.length > 0)
    {
        if(confirm('Kamu Yakin Mau Hapus Data '+list_id.length+' ini ?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "<?php echo site_url('master/jenisrekeningbiaya/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
                       ...
                       <div class="row">
                         <div class="form-group col-6 mb-2">
                            ...
                            <div class='input-group'>
                              <input type='hidden' class="form-control form-control-lg input-lg" name="id_jns_rekbiaya" />
                               <input type='text' class="form-control form-control-lg input-lg" name="id_jns_rekbiayaa" min="1" max="99" readonly required="" />
                            </div>
                          </div>
                          <div class="col-6 mb-2">
                             ...
                             <div class="input-group">
                              ...
                              <input type="text" class="form-control form-control-lg input-lg position-inside-maxlength"
                               id="maxlength-position-inside" maxlength="30" name="nm_jns_rekbiaya" required="" placeholder="" aria-describedby="basic-addon3">
                            </div>
                           </div>
