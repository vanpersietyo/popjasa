    ...
    <div class="row">
      <div class="col-lg-6 col-md-6">
        <div class="form-group">
          <button type="button" class="btn mb-1 btn-info btn-lg btn-block pull-up" onclick="add_person()"> <a> <i class="la la-plus"></i> Tambah Potongan Baru </a></button>
        </div>
      </div>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('master/hrd/potongan/ajax_list')?>",
            "type": "POST"
        },
...
function add_person()
{
    ...
    $('.modal-title').text('Tambah Potongan Baru'); // Set Title to Bootstrap modal title
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('master/hrd/potongan/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.id_potongan);
            $('[name="id_potongan"]').val(data.id_potongan);
            $('[name="nm_potongan"]').val(data.nm_potongan);
            $('[name="keterangan"]').val(data.keterangan);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Potongan'); // Set title to Bootstrap modal title
        },
...
function save()
{
  clear_all_error();
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master/hrd/potongan/ajax_add')?>";
    } else {
        url = "<?php echo site_url('master/hrd/potongan/ajax_update')?>";
    }
...
function delete_person(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('master/hrd/potongan/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
                     ...
                     <div class="form-body">
                       ...
                       <div class="form-group">
                          <div class="row">
                           ...
                           <div class="col-md-12">
                               <input name="nm_potongan" placeholder="Nama Potongan .." class="form-control" type="text">
                               <div class="NOTIF_ERROR_nm_potongan"></div>
                           </div>
                          </div>
