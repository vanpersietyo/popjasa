...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('master/hrd/trans_potongan/ajax_list')?>",
                "type": "POST"
            },
        ...
        table2 = $('#table2').DataTable({
            ...
            // Load data for the table's content from an Ajax source
            "ajax": {
                "url": "<?php echo site_url('master/hrd/trans_potongan/ajax_list2')?>",
                "type": "POST"
            },
    ...
    function edit_person(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('master/hrd/trans_potongan/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                ...
                $('.modal-title').text('Tambah Potongan Karyawan'); // Set title to Bootstrap modal title
            },
    ...
    function delete_person(id) {
        if (confirm('Anda Yakin Akan Menghapus Data?')) {
            ...
            $.ajax({
                url: "<?php echo site_url('master/hrd/trans_potongan/ajax_delete')?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
    ...
    function save() {
        ...
        if (save_method == 'add') {
            url = "<?php echo site_url('master/hrd/trans_potongan/ajax_add')?>";
        } else {
            url = "<?php echo site_url('master/hrd/trans_potongan/ajax_update')?>";
        }
...
<script type="text/javascript">
    function detail(ID) {
        location.replace("<?php echo base_url('master/hrd/trans_potongan/history/')?>" + ID);
    }

    function konfirmasi() {
        location.replace("<?php echo base_url('master/hrd/trans_potongan/konfirmasi/' . $this->session->userdata('yangLogin'))?>");
    }
