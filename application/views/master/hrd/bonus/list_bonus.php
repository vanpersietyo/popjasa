  ...
  <div class="content-wrapper">
    <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2">
          <h3 class="content-header-title">Pemberian Bonus Karyawan</h3>
          <div class="row breadcrumbs-top">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><p class="danger">Modul ini di gunakan untuk melakukan transaksi pemberian bonus pada karyawan</p>
                </li>
              </ol>
            </div>
          </div>
      ...
      <section class="row">
        ...
        <div class="col-6">
          <div class="card">
            <div class="card-content">
              <div class="card-body">
                <h3 class="content-header-title">Daftar Penerima Bonus Periode :<?php echo date('M Y');?></h3>
                <br>
                <!-- Invoices List table -->
                <div class="table-responsive">
                  <table id="table2" class="table table-striped table-bordered sourced-data">
                    ...
                    <tfoot>
                      <tr>
                          <th colspan="1"><h5 class="text-bold-500" align="right">Total Bonus:<h5></th>
                          <th colspan="2"></th>
                      </tr>
                  </tfoot>
                ...
                <div class="row">
                  <div class="col-md-12">
                      <a type="button" href="<?php echo site_url('master/hrd/bonus_karyawan/konfirmasi/'.$this->session->userdata('yangLogin'));?>" class="btn mb-1 btn-info box-shadow-2 btn-lg btn-block pull-up"> Konfirmasi</a>
                  </div>
                </div>
                <!--/ Invoices table -->
              </div>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('master/hrd/bonus_karyawan/ajax_list')?>",
            "type": "POST"
        },
    ...
    table2 = $('#table2').DataTable({
        ...
        // Load data for the table's content from an Ajax source
        "ajax": {
            "url": "<?php echo site_url('master/hrd/bonus_karyawan/ajax_list2')?>",
            "type": "POST"
        },
...
{
    $('[name="jumlah_bonus"]').removeClass('border-danger');
    $('[class="NOTIF_ERROR_jumlah_bonus"]').html('');
}
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('master/hrd/bonus_karyawan/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function delete_person(id)
{
    if(confirm('Anda Yakin Akan Menghapus Data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('master/hrd/bonus_karyawan/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master/hrd/bonus_karyawan/ajax_add')?>";
    } else {
        url = "<?php echo site_url('master/hrd/bonus_karyawan/ajax_update')?>";
    }
...
<script type="text/javascript">
  function detail(ID){
     location.replace("<?php echo base_url('master/hrd/bonus_karyawan/history/')?>"+ID);
   }

  function konfirmasi(){
        location.replace("<?php echo base_url('master/hrd/bonus_karyawan/konfirmasi/'.$this->session->userdata('yangLogin'))?>");
      }
                           ...
                           <div class="row">
                            ...
                            <div class="col-md-6">
                             ...
                             <select class="form-control" name="id_jabatan" disabled>
                              ...
                              <div class="col-md-12">
                                <label for="issueinput5">Jumlah Bonus</label>
                                <input type="text" name="jumlah_bonus" class="form-control" id="tanpa-rupiah" required="" >
                                   <div class="NOTIF_ERROR_jumlah_bonus"></div>
                              </div>
