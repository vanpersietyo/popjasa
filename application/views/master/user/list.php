    ...
    <div class="row">
      <div class="col-lg-6 col-md-6">
        <div class="form-group">
          <button type="button" class="btn mb-1 btn-info btn-lg btn-block pull-up" onclick="add_person()"> <a> <i class="la la-plus"></i> Tambah User Baru </a></button>
        </div>
      </div>
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('master/user/ajax_add')?>";
    } else {
        url = "<?php echo site_url('master/user/ajax_update')?>";
    }
    ...
    if(list_id.length > 0)
    {
        if(confirm('Kamu Yakin Mau Hapus Data '+list_id.length+' ini ?'))
        {
            $.ajax({
                type: "POST",
                data: {id:list_id},
                url: "<?php echo site_url('master_produk/kategori/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
                            ...
                            <div class="col-md-12">
                             ...
                             <select class="form-control" name="id_karyawan" required>
                                ...
                                <?php foreach ($employee as $employee) { ?>
                                  <option value="<?php echo $employee->id_karyawan ?>"> <?php echo "$employee->nama_karyawan" ?> </option>
                                <?php }?>
                              </select>
                              </div>
