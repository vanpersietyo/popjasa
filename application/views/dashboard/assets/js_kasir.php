...
$(window).on("load", function(){
    ...
    ********************************************/
    ...
    Morris.Bar({
        ...
        data: [<?php echo $grafik_omzet_group_ktg?>],
        xkey: 'NM_KTGPRD',
        ykeys: ['OMZET'],
        labels: ['OMZET'],
        barGap: 4,
...
$('#top_penjualan_jasamurah_filter').html('');
$('#search_penjualan_jasamurah').on('keyup',function(){
...
function detail_hutang(id) {
  window.open("http://paceandrace.qloud.id/transaksi/keuangan/hutang/detail/"+id);
}

function detail_piutang(id) {
  window.open("http://paceandrace.qloud.id/transaksi/keuangan/piutang/detail/"+id);
}
</script>
