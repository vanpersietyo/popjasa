...
function add_person(){
    save_method = 'add';
    $('[name="<?=M_bankout::KD_BANK?>"]').val('').trigger('change');
    ...
    $('.modal-title').text('Tambah Transaksi Kas Keluar'); // Set Title to Bootstrap modal title
}
...
function edit_person(id)
{
    ...
    $.ajax({
        ...
        {
            $('[name="<?=M_bankout::ID_TRANS;?>"]').val(data.<?=M_bankout::ID_TRANS;?>);
            $('[name="<?=M_bankout::KD_BANK;?>"]').val(data.<?=M_bankout::KD_BANK;?>).trigger('change').prop('disabled',false);
            $('[name="<?=M_bankout::SLD_KELUAR;?>"]').val(formatRupiah(data.<?=M_bankout::SLD_KELUAR;?>)).prop('disabled',false);
            $('[name="<?=M_bankout::KETERANGAN;?>"]').val(data.<?=M_bankout::KETERANGAN;?>).prop('disabled',false);
            $('[name="<?=M_bankout::TGL_TRANS;?>"]').val(formatDate(data.<?=M_bankout::TGL_TRANS;?>,'2')).prop('disabled',false);
            $('#btnSave').prop('disabled',false);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Kas Masuk ' + data.<?=M_bankout::ID_TRANS;?>); // Set title to Bootstrap modal title
        },
...
function lookup(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            $('[name="<?=M_bankout::KD_BANK;?>"]').val(data.<?=M_bankout::KD_BANK;?>).trigger('change').prop('disabled',true);
            $('[name="<?=M_bankout::SLD_KELUAR;?>"]').val(formatRupiah(data.<?=M_bankout::SLD_KELUAR;?>)).prop('disabled',true);
            $('[name="<?=M_bankout::KETERANGAN;?>"]').val(data.<?=M_bankout::KETERANGAN;?>).prop('disabled',true);
            $('[name="<?=M_bankout::TGL_TRANS;?>"]').val(formatDate(data.<?=M_bankout::TGL_TRANS;?>,'2')).prop('disabled',true);
            $('#btnSave').prop('disabled',true);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Lihat Kas Keluar ' + data.<?=M_bankout::ID_TRANS;?>); // Set title to Bootstrap modal title
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error get data from ajax');
...
function clear_all_error()
{
    ...
    $('[class="NOTIF_ERROR_TGL_TRANS"]').html('');
