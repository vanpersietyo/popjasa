    ...
    public function json2()
    {
        ...
        if ($list) {
            foreach ($list as $i => $detail) {
                ...
                $row[] = $detail->Date_FA;
                $row[] = $detail->Harga;
                $row[] = '<a class="btn btn-sm btn-success" href="javascript:void(0)" title="Tambah FA" onclick="pilih_fix_asset(' . "'" . $detail->Fa_ID . "','" . $detail->Nama_FA . "','" . $detail->Date_FA . "','" . $detail->Harga . "'" . ')"><i class="ft-plus-square"></i></a>';
