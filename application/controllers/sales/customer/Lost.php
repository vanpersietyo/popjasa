...
class Lost extends CI_Controller{
	...
	public function index(){
		...
		$data['pages']='sales/customer/list_lost';
	...
	public function ajax_list()
	{
		...
		$list = $this->M_Customer->get_user_lost();
		$data = array();
		...
		foreach ($list as $d) {
			...
			$row[] = '<button type="button" class="btn btn-dark dropdown-toggle btn-sm" data-toggle="dropdown"
			                      ...
			                      <div class="dropdown-menu">
															<a class="dropdown-item" href="javascript:void(0)" onclick="deals('."'".$d->id_customer."'".')"><i class="ft-thumbs-up"></i> Generate To Deal</a>
		...
		$output = array(
						...
						"recordsTotal" => $this->M_Customer->count_all_lost(),
						"recordsFiltered" => $this->M_Customer->count_filtered_lost(),
						"data" => $data,
				);
