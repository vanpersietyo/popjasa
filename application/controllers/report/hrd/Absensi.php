		...
		$sysdate=date('d/m/Y H:i');
	        ...
	        //REKAPITULASI
	        	...
	        	$pdf->Cell(10,5,'',0,1);
				 ...
				 $pdf->Cell(10,5,'',0,1);
				 $pdf->SetFont('Arial','B',8);
						 $pdf->Cell(280,5,"Detail Daftar Kehadiran, $get_karyawan->nama_karyawan",0,2,'L');
						 $pdf->SetFont('Arial','',7);
						 $pdf->Cell(10,5,'No',1,0,'C');
						 $pdf->Cell(110,5,'Nama',1,0,'L');
						 $pdf->Cell(35,5,'Tgl Absen',1,0,'C');
						 $pdf->Cell(35,5,'Status Absen',1,0,'C');
							 $pdf->Cell(10,5,'',0,1);
							 $detail = $this->M_absensi_karyawan->report_absensi_id($TGL01,$TGL02,$id_karyawan);
							 $d='1';
							 foreach ($detail as $detail){
								$pdf->SetFont('Arial','',7);
								if ($detail->status=='M') {
